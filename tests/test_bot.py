...
@pytest.mark.asyncio
@patch('bot.fetch_new_articles', return_value=[]) # fetch_new_articles теперь возвращает пустой список
async def test_check_news_parser_returns_none(mock_fetch_articles, mock_context):
    """Тест: обработка случая, когда парсер не находит статей."""
    mock_context.job.user_id = 12345
    await check_and_post_news(mock_context)
    mock_context.bot.send_message.assert_called_once_with(chat_id=12345, text="Новых статей не найдено.")
