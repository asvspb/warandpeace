...
**Стек технологий:**
- **Язык:** Python 3
- **Библиотеки:** `python-telegram-bot`, `requests`, `beautifulsoup4`, `python-dotenv`, `google-generativeai`, `feedparser`, `tenacity`, `Flask`.
...
- **`src/`**: Исходный код.
    ...
    - **`parser.py`**: Парсинг статей с сайта.
    - **`summarizer.py`**: Суммирование текста с помощью AI.
    - **`healthcheck.py`**: Flask-приложение для проверки состояния.
    - **`telegram_token_checker.py`**: Утилита для проверки токена Telegram.
- **`database/`**: Хранилище базы данных.
- **`tests/`**: Модульные тесты.
- **`temp/`**: Временные файлы.
- **`devAI/`**: Каталог для файлов, связанных с разработкой AI.
...
7.  **Формат сообщения:**
    ...
    -   Резюме (~150 слов) от `summarizer.py` (используется Google Gemini с резервным переключением на OpenRouter).
...
10. **Парсинг RSS (не используется):** В коде присутствует функция для получения новостей через RSS-ленту, однако она неактивна, так как RSS на исходном сайте работает некорректно.
...
- **Сохранение плана:** Всякий раз, когда предлагается план чего-либо, он должен быть сохранен в отдельном документе `devAI/PLANNING.md`.
- **Корректировка плана:** В ходе реализации плана в этот документ должны вноситься соответствующие коррективы.
- **Улучшение плана:** Если пользователь предлагает план, его необходимо улучшить и внести в `devAI/PLANNING.md`.
- **Ознакомление с проектом:** В каждой новой сессии необходимо ознакомиться с текущим состоянием проекта, включая `devAI/PLANNING.md`.
