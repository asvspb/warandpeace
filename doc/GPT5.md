...
### Правило ведения CHANGELOG и формирования RELEASELOG
- На ветке `develop` необходимо поддерживать в актуальном состоянии `doc/CHANGELOG.md` (секция Unreleased с блоками: Added, Changed, Fixed, Docs, Infra, Security). Записывать только пользовательски значимые изменения и кратко «зачем».
- Перед слиянием в `main` сформировать релиз:
  - Пронумеровать релиз (семвер) и поставить дату в `CHANGELOG.md`; очистить/перенести Unreleased для следующего цикла.
...
### Проверки после слияния (обязательно)
- Немедленно после слияния в целевую ветку прогнать тесты: `pytest -q`.
- Проверить актуальные теги версии: `git tag --list | sort -V | tail -n 5` и убедиться, что создан и запушен тег релиза (`git push --tags`).
- При наличии CI — убедиться, что все проверки зелёные.
- Пересобрать Docker-образ и проверить логи на ошибки:
  - `docker-compose build && docker-compose up -d`
  - `docker-compose logs --tail=200` (убедиться в отсутствии ошибок/traceback)
