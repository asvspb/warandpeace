...
- [x] Инъекция сетевой ошибки (для удалённых): убедиться в ретраях и корректном fail‑логировании.
- [x] Ротация: сгенерировать бэкапы за 10 дней → проверить политику очистки.
- [x] Шифрование: проверить недоступность содержимого без ключей.
- [x] Производительность: измерить длительность; локальная выгрузка должна быть fastest.
