...
- [ ] CLI для DLQ: `dlq-show`, `dlq-retry` (c бюджетами ретраев)
- [ ] Использование `content_hash` для поиска «почти-дубликатов» и отчётов
- [ ] Экспорт метрик DLQ/ingest из CLI/джобов (регулярные обновления)
- [ ] Транзакционный бэкап/rollback миграций
- [ ] Docker: публикация порта метрик и переменные `METRICS_ENABLED`/`METRICS_PORT`