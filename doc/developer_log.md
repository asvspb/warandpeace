...
### 2025-08-08 - release: Версия 1.3.0

**Описание:**
Выпущен новый минорный релиз, включающий в себя две важные функции для администрирования и монетизации бота.

**Ключевые изменения:**
1.  **Поддержка нескольких администраторов:** Реализована возможность указывать несколько ID администраторов в `TELEGRAM_ADMIN_IDS`, что повышает гибкость управления.
2.  **Премиум-канал для дайджестов:** Вместо сложной и ненадежной рассылки по личным сообщениям, дайджесты теперь публикуются в выделенный (возможно, приватный) канал (`PREMIUM_DIGEST_CHANNEL_ID`). Это улучшает пользовательский опыт и упрощает управление доступом.

---

### 2025-08-07 - refactor(premium): Переход на премиум-канал для дайджестов

**Описание:**
Система доставки премиум-контента была полностью переработана. Вместо неудобной рассылки по списку ID, которая требовала от пользователей активации бота, теперь используется публикация в выделенный (возможно, приватный) Telegram-канал. Это значительно улучшает пользовательский опыт и упрощает управление доступом.
...
### 2025-08-07 - feat(premium): Реализована рассылка дайджестов премиум-пользователям

**Описание:**
Добавлена новая функция для премиум-пользователей: ручная рассылка аналитических дайджестов. Администратор может сгенерировать дайджест, который будет автоматически отправлен всем пользователям из премиум-списка.

**Изменения:**
1.  **`src/config.py`**:
    *   Добавлена новая переменная окружения `PREMIUM_USER_IDS` для хранения списка ID премиум-пользователей.
2.  **`src/bot.py`**:
    *   Создана новая асинхронная функция `send_digest_to_subscribers` для безопасной рассылки сообщений по списку ID с обработкой ошибок.
    *   Все команды-обработчики дайджестов (`/daily_digest` и т.д.) были обновлены: теперь после генерации сводки они вызывают функцию рассылки.
    *   Администратор, запустивший команду, получает отчет о количестве успешных отправок и предпросмотр дайджеста.
3.  **`.env.example`**:
    *   Добавлена переменная `PREMIUM_USER_IDS` с описанием.

---

### 2025-08-07 - feat(auth): Реализована поддержка нескольких администраторов

**Описание:**
Проведен рефакторинг системы аутентификации для поддержки нескольких администраторов. Это повышает гибкость управления ботом и отказоустойчивость.

**Изменения:**
1.  **`src/config.py`**:
    *   Логика загрузки переменных окружения обновлена для парсинга `TELEGRAM_ADMIN_IDS` — строки с ID, разделенными запятой, — в список целочисленных ID.
2.  **`src/bot.py`**:
    *   Обработчики команд и функция `post_init` (для установки меню команд) теперь используют список `TELEGRAM_ADMIN_IDS` вместо одного ID.
    *   Добавлено логирование успешной установки команд для каждого администратора.
3.  **Документация:**
    *   Обновлены `.env.example` и `DEPLOYMENT.md` для отражения нового формата переменной `TELEGRAM_ADMIN_IDS`.
