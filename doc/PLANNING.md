...
**Цель:** Привести конфигурацию, код и документацию в соответствие, устранив выявленные расхождения.

### Этап 1: Унификация переменных окружения
-   `[X]` **`src/config.py`**:
    -   `[X]` Сделать `TELEGRAM_ADMIN_ID` опциональным (не вызывать ошибку при отсутствии).
    -   `[X]` Добавить поддержку `TELEGRAM_ADMIN_IDS` в формате CSV для списка администраторов.
    -   `[X]` Реализовать парсинг `GOOGLE_API_KEYS` в формате CSV, сохранив обратную совместимость с `GOOGLE_API_KEY_n`.
-   `[ ]` **`.env.example`**: Обновить, добавив `TELEGRAM_ADMIN_IDS` и `GOOGLE_API_KEYS` и отметив `TELEGRAM_ADMIN_ID` как опциональный.
-   `[ ]` **`DEPLOYMENT.md`**: Синхронизировать документацию с изменениями в `config.py` и `.env.example`.

### Этап 2: Актуализация кодовой базы
-   `[X]` **`src/get_todays_articles.py`**: Проанализировать и, скорее всего, удалить как устаревший, чтобы избежать путаницы.
-   `[ ]` **`src/notifications.py` (или аналог)**: Найти код, использующий `TELEGRAM_ADMIN_IDS`, и адаптировать его к новой конфигурации.
-   `[X]` **`requirements.txt`**: Проверить и при необходимости обновить версию `google-generativeai` для поддержки актуальных моделей.

### Этап 3: Тестирование
-   `[X]` Создать или обновить тесты для `config.py` для проверки:
    -   `[X]` Отсутствия падения при нехватке `TELEGRAM_ADMIN_ID`.
    -   `[X]` Корректного парсинга `TELEGRAM_ADMIN_IDS` и `GOOGLE_API_KEYS`.

### Этап 4: Завершение
-   `[ ]` Обновить `doc/developer_log.md`.
-   `[ ]` Сделать коммит с внесенными изменениями.

---

# План перехода на модель `models/gemini-2.0-flash-thinking-exp-01-21`

### Этап 1: Анализ и рефакторинг
- [ ] **Изучить документацию:** Найти официальную документацию по модели `gemini-2.5-flash`. Убедиться, что не изменились требования к API, параметры запроса или необходимая версия библиотеки `google-generativeai`.
- [X] **Централизовать имя модели:** Вынести имя модели из кода в файл конфигурации, чтобы избежать дублирования и упростить будущие обновления.
    - [X] **Действие:** В `src/config.py` добавить новую переменную `GEMINI_MODEL_NAME`, которая будет загружаться из `.env`.
    - [X] **Действие:** В `src/summarizer.py` заменить жестко закодированную строку 'gemini-1.5-flash-latest' на импорт и использование `config.GEMINI_MODEL_NAME`.
    - [X] **Действие:** В `src/telegram_token_checker.py` также заменить жестко закодированную модель на `config.GEMINI_MODEL_NAME`.
    - [X] **Действие:** Добавить `GEMINI_MODEL_NAME="models/gemini-1.5-flash-latest"` в файл `.env.example` в качестве значения по умолчанию.

### Этап 2: Переход и тестирование
- [X] **Обновить `.env`:** В локальном файле `.env` установить для переменной `GEMINI_MODEL_NAME` новое значение: `models/gemini-2.0-flash-thinking-exp-01-21`.
- [ ] **Проверить зависимости:** Убедиться, что текущая версия `google-generativeai` в `requirements.txt` совместима с новой моделью. При необходимости обновить.
- [X] **Провести ручное тестирование:** Убедиться, что основные функции, использующие модель, работают корректно с новым именем.
    - [X] **Действие:** Запустить `python src/summarizer.py` и проверить, что тестовое суммирование проходит успешно.
    - [X] **Действие:** Запустить `python src/telegram_token_checker.py` и убедиться, что проверка ключей API проходит для новой модели.
    - [X] **Действие:** Запустить `python manage.py backfill --limit 1` для проверки полного цикла обработки статьи.
- [X] **Запустить юнит-тесты:** Выполнить команду `pytest` и убедиться, что все тесты по-прежнему проходят после рефакторинга.

### Этап 3: Завершение
- [X] **Обновить документацию:** Проверить `README.md` и `BEST_PRACTICES.md` на предмет упоминания старой модели и при необходимости обновить.
- [X] **Зафиксировать изменения:** Сделать коммит с результатами рефакторинга и перехода на новую модель.
