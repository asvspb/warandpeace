...
- [ ] **Задача:** Убедиться, что изменения работают корректно и не сломали существующую функциональность.
- [ ] **Действие:**
    - [X] Запустить тесты с помощью `pytest`.
    - [ ] Проверить логи в реальном времени, чтобы убедиться, что новые сообщения появляются, а старые - нет.
