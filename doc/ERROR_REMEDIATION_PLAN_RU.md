...
- [X] **Кэширование статуса ключей**: при 400 `User location is not supported` помечать ключ как `geo_unsupported` и исключать из перебора на 24 часа (in-memory + файл `temp/gemini_key_status.json`).
 ...
 - [X] **Исправить баг с 429 (quota)**: сейчас обработчик `ResourceExhausted` недостижим из-за размещения после `except Exception`. Нужно:
  - [X] Перестроить блок `try/except` так, чтобы `except ResourceExhausted` шёл раньше общего `except Exception` и действительно срабатывал.
  - [X] Заполнять `cooldown_until` с учётом `retry_delay` из ответа (если доступно), минимум 1–5 минут.
  - [X] После пометки ключа — продолжать перебор следующего ключа в рамках того же вызова (не `return None` преждевременно).
 - [X] **Стабильный идентификатор ключа**: вместо `hash(api_key)` использовать `sha256(api_key).hexdigest()` для стабильности между процессами/перезапусками.
 - [X] **Не прерывать перебор при geo_unsupported**: помечать ключ и сразу пробовать следующий в текущем вызове (не выходить `return None`), чтобы не тратить попытку на каждом артикле.
...
- [ ] **Политика provider order**: если активных ключей Gemini нет (или доля успехов < порога), на 6 часов делать Mistral первичным (`LLM_PRIMARY=mistral` в рантайме либо авто‑переключатель).
 - [X] **Логирование**: понизить ожидаемые ошибки Gemini (400/429) до WARNING и агрегировать: одно сообщение на запуск, без повторов на каждую статью.
