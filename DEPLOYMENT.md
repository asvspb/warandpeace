...
Веб-интерфейс работает отдельным сервисом `web` и получает доступ к БД в режиме read-only. По умолчанию доступен на `http://<host>:8080`.
...
- Healthcheck доступен на `/healthz`. Метрики Prometheus — на `/metrics` (публично).
