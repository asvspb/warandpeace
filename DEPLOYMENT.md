...
Перед первым запуском необходимо убедиться, что у Docker есть права на запись в каталог с базой данных. Выполните следующую команду в корне проекта на вашем сервере:

```bash
mkdir -p database && sudo chown -R $(id -u):$(id -g) database
```

Эта команда создаст папку `database`, если она не существует, и установит для нее правильного владельца, что позволит избежать ошибок доступа к файлу `articles.db` внутри контейнера.
