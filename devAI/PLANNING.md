# План перехода на модель `gemini-2.5-flash`
...
- [ ] **Централизовать имя модели:** Вынести имя модели из кода в файл конфигурации, чтобы избежать дублирования и упростить будущие обновления.
    - [ ] **Действие:** В `src/config.py` добавить новую переменную `GEMINI_MODEL_NAME`, которая будет загружаться из `.env`.
    - [ ] **Действие:** В `src/summarizer.py` заменить жестко закодированную строку `'gemini-1.5-flash-latest'` на импорт и использование `config.GEMINI_MODEL_NAME`.
    - [ ] **Действие:** В `src/telegram_token_checker.py` также заменить жестко закодированную модель на `config.GEMINI_MODEL_NAME`.
    - [ ] **Действие:** Добавить `GEMINI_MODEL_NAME="models/gemini-1.5-flash-latest"` в файл `.env.example` в качестве значения по умолчанию.

### Этап 2: Переход и тестирование
- [ ] **Обновить `.env`:** В локальном файле `.env` установить для переменной `GEMINI_MODEL_NAME` новое значение: `models/gemini-2.5-flash`.
...
- [ ] **Провести ручное тестирование:** Убедиться, что основные функции, использующие модель, работают корректно с новым именем.
    - [ ] **Действие:** Запустить `python src/summarizer.py` и проверить, что тестовое суммирование проходит успешно.
    - [ ] **Действие:** Запустить `python src/telegram_token_checker.py` и убедиться, что проверка ключей API проходит для новой модели.
    - [ ] **Действие:** Запустить `python manage.py backfill --limit 1` для проверки полного цикла обработки статьи.
- [ ] **Запустить юнит-тесты:** Выполнить команду `pytest` и убедиться, что все тесты по-прежнему проходят после рефакторинга.

### Этап 3: Завершение
- [ ] **Обновить документацию:** Проверить `README.md` и `BEST_PRACTICES.md` на предмет упоминания старой модели и при необходимости обновить.
- [ ] **Зафиксировать изменения:** Сделать коммит с результатами рефакторинга и перехода на новую модель.
