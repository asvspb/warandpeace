### 2025-08-06 - Синхронизация документации с кодовой базой

**Описание:**
Проведена полная ревизия документации (`devAI/GEMINI.md`, `DEPLOYMENT.md`) и приведена в соответствие с текущим состоянием проекта. Устранены расхождения, добавлена информация о новых возможностях и зависимостях.

**Изменения:**
1.  **`devAI/GEMINI.md` обновлен:**
    *   **Стек технологий:** Добавлены библиотеки `feedparser`, `tenacity` и `Flask`.
    *   **Структура проекта:** Добавлено описание для утилиты `src/telegram_token_checker.py`.
    *   **Логика работы:**
        *   Уточнено, что для суммирования используется Google Gemini с резервным переключением на OpenRouter.
        *   Добавлена информация о том, что функция парсинга RSS существует, но не используется из-за неработающей RSS-ленты на сайте-источнике.
2.  **`DEPLOYMENT.md` обновлен:**
    *   В инструкции по настройке `.env` файла уточнено, что можно указывать несколько ключей для Google API.
